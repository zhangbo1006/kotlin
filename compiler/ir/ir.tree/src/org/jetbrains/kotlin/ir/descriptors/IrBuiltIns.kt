...
//        builtIns.array to IrTypeMapper({ arrayType }, { buildNullableType(arrayType) })
    ...
    val lessFunByOperandType = primitiveTypesWithComparisons.defineComparisonOperatorForEachType(OPERATOR_NAME_LESS)
    val lessOrEqualFunByOperandType = primitiveTypesWithComparisons.defineComparisonOperatorForEachType(OPERATOR_NAME_LESS_OR_EQUAL)
    val greaterOrEqualFunByOperandType = primitiveTypesWithComparisons.defineComparisonOperatorForEachType(OPERATOR_NAME_GREATER_OR_EQUAL)
    val greaterFunByOperandType = primitiveTypesWithComparisons.defineComparisonOperatorForEachType(OPERATOR_NAME_GREATER)

    val ieee754equalsFunByOperandType =
        primitiveFloatingPointTypes.associate {
            it to defineOperator(OPERATOR_NAME_IEEE754_EQUALS, bool, listOf(it.makeNullable(), it.makeNullable()))
        }

    val eqeqeqFun = defineOperator(OPERATOR_NAME_EQEQEQ, bool, listOf(anyN, anyN))
    val eqeqFun = defineOperator(OPERATOR_NAME_EQEQ, bool, listOf(anyN, anyN))
    val throwNpeFun = defineOperator(OPERATOR_NAME_THROW_NPE, nothing, listOf())
    val throwCceFun = defineOperator(OPERATOR_NAME_THROW_CCE, nothing, listOf())
    val throwIseFun = defineOperator(OPERATOR_NAME_THROW_ISE, nothing, listOf())
    val booleanNotFun = defineOperator(OPERATOR_NAME_NOT, bool, listOf(bool))
    val noWhenBranchMatchedExceptionFun = defineOperator(OPERATOR_NAME_NO_WHEN_BRANCH_MATCHED_EXCEPTION, nothing, listOf())
    val illegalArgumentExceptionFun = defineOperator(OPERATOR_NAME_ILLEGAL_ARGUMENT_EXCEPTION, nothing, listOf(string))
