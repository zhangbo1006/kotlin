...
class SwitchGenerator(private val expression: IrWhen, private val data: BlockInfo, private val codegen: ExpressionCodegen) {
    ...
    fun generate(): StackValue? {
        ...
        if (areConstIntComparisons(calls)) {
            return IntSwitch(subject, defaultLabel, endLabel, elseExpression, expressionToLabels, cases).genOptimizedIfEnoughCases()
        } else if (areConstStringComparisons(calls)) {
            return StringSwitch(subject, defaultLabel, endLabel, elseExpression, expressionToLabels, cases).genOptimizedIfEnoughCases()
        }
