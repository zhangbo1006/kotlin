...
object CheckDeclarationParentsVisitor : IrElementVisitor<Unit, IrDeclarationParent?> {
    ...
    private fun checkParent(declaration: IrDeclaration, expectedParent: IrDeclarationParent?) {
        val parent = try {
            ...
            error("$declaration for ${declaration.descriptorWithoutAccessCheck} has no parent") ///
        }

        if (parent != expectedParent) {
            error("$declaration for ${declaration.descriptorWithoutAccessCheck} has unexpected parent $parent")
        }
    }
}
