...
object ModuleWrapperTranslation {
    ...
    object Namer {
        fun requiresEscaping(moduleId: String) = false
    }
    ...
    private fun wrapAmd(
        ...
        val invocationArgs = listOf(
            JsArrayLiteral(listOf(JsStringLiteral("exports")) + importedModules.map { JsStringLiteral(it) }),
            function
        )
    ...
    private fun wrapCommonJs(
        ...
        val invocationArgs = importedModules.map { JsInvocation(requireName.makeRef(), JsStringLiteral(it)) }
    ...
    private fun addModuleValidation(
        ...
        val moduleNotFoundMessage = JsStringLiteral(
            "Error loading module '" + currentModuleId + "'. Its dependency '" + module + "' was not found. " +
                    "Please, check whether '" + module + "' is loaded prior to '" + currentModuleId + "'."
        )
