...
open class FirTypeResolveTransformer(
    private val traversedClasses: Set<FirClass> = setOf()
    ...
    private fun resolveSuperTypesAndExpansions(element: FirMemberDeclaration) {
        try {
            element.transformChildren(SuperTypeResolver(traversedClasses + listOfNotNull(element as? FirClass)), null)
        } catch (e: Exception) {
    ...
    private class SuperTypeResolveTransformer(
        ...
        traversedClasses: Set<FirClass>
    ) : FirTypeResolveTransformer(traversedClasses) {
        override fun <E : FirElement> transformElement(element: E, data: Nothing?): CompositeTransformResult<E> {
    ...
    private inner class SuperTypeResolver(val traversedClasses: Set<FirClass>) : FirTransformer<Nothing?>() {
        override fun <E : FirElement> transformElement(element: E, data: Nothing?): CompositeTransformResult<E> {
            return element.compose()
        }
            ...
            if (symbol is ConeClassLikeSymbol) {
                if (symbol is FirBasedSymbol<*>) {
                    ...
                    val transformer = SuperTypeResolveTransformer(
                        firElementsToVisit.iterator(), traversedClasses
                    )
        ...
        override fun transformType(type: FirType, data: Nothing?): CompositeTransformResult<FirType> {
            ...
            if (symbol != null) {
                if (symbol is FirClassSymbol && symbol.fir in traversedClasses) {
